    ...
    def flush(timeout = 10):
        ...
        with LogManagerImpl.logger_lock:
            ...
            for i in LogManagerImpl.__loggers:
                if LogManagerImpl.__loggers[i]._AriaLogger__tenant_token != StatsConstants.stats_tenant_token:
                    LogManagerImpl.__loggers[i]._AriaLogger__flush()
    ...
    def __create_or_get_logger(source, tenantToken):
        ...
        try:
            if (source, tenantToken) in LogManagerImpl.__loggers:
                ...
                logger_local = aria_logger.AriaLogger(source, tenantToken)
